...
Grammer of CSS 3 specification
...
import
  : IMPORT_SYM space_or_empty string_or_uri space_or_empty media_query_list ';' space_or_empty
    %{
      $$ = {}
      if ($3 !== null) $$["import"] = $3;
...
/* 
CSS2.1 media_query_list -> [ medium [ ',' S* medium]* ]? 
CSS3.0 media_query_list -> [ medium [ ',' S* medium]* ]*
...
/* 
CSS2.1 medium -> IDENT S* 
CSS3.0 medium -> [ "not" | "only" ]? media_type [ "and" expression ]* S*
*/
medium
  : media_type expression_list space_or_empty
    %{
      $$ = {}
      $$.prefix = '';
      $$.media_type = $1;
      $$.expression = $2 !== null? $2:'';
    %}
  | media_type_prefix media_type expression_list space_or_empty
    %{
      $$ = {}
      $$.prefix = $1;
      $$.media_type = $2;
...
/* CSS3.0 */
media_type_prefix
  : MEDIA_TYPE_PREFIX space_or_empty -> $1
  ;

/* CSS3.0 expression_list -> [ "and" S+ expression ]* */
expression_list
  : MEDIA_TYPE_AND at_least_one_space expression
    %{
      $$ = [];
      $$.push($3);
    %}
  | expression_list MEDIA_TYPE_AND at_least_one_space expression
    %{
      $$ = [];
      if ($1 !== null) $$ = $1;
      if ($4 !== null) $$.push($4);
...
/* CSS3.0 expression -> '(' media_feature [: S+ value]? ')' */
expression
  : '(' media_feature ')' space_or_empty
    %{
      $$ = {}
      $$.media_feature = $2;
      $$.value = '';
    %}
  | '(' media_feature ':' space_or_empty term ')' space_or_empty // TODO: is value = term?
    %{
      $$ = {}
      $$.media_feature = $2;
      $$.value = $5;
